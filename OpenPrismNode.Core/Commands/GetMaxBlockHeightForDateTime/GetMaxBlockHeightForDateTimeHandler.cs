...
public class GetMaxBlockHeightForDateTimeHandler : IRequestHandler<GetMaxBlockHeightForDateTimeRequest, Result<int>>
{
    ...
    {
        request.VersionTime = DateTime.SpecifyKind(request.VersionTime, DateTimeKind.Unspecified);
        var maxBlockHeight = await _context.BlockEntities
            .Where(b => b.IsFork == false &&
                        b.EpochEntity.Ledger == request.Ledger &&
                        b.TimeUtc <= request.VersionTime && b.TimeUtc > DateTime.MinValue)
        ...
        {
            return Result.Fail($"No blocks found in the database within the given constraint TimeUtc <= {request.VersionTime}");
        }
