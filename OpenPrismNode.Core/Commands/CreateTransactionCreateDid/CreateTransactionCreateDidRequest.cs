...
/// Request
/// </summary>
public class CreateTransactionCreateDidRequest : TransactionBaseRequest, IRequest<Result<TransactionModel?>>
{
    /// <summary>
    /// Request Constructor
    /// </summary>
    ...
    public CreateTransactionCreateDidRequest(Hash transactionHash, Hash blockHash, int blockHeight, int fees, int size, int index, Hash operationHash, string did, string signingKeyId,
        int operationSequenceNumber, List<UtxoWrapper> utxos, List<PrismPublicKey> prismPublicKeys, List<PrismService> prismServices)//, int operationSequenceNumber)//, List<PrismUtxoModel> incomingUtxos, List<PrismUtxoModel> outgoingUtxos)
